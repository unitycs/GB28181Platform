...
static int get_REG_SZ(HKEY hKey, const char *leafKeyName, char **outptr)
{
	...
	*outptr = NULL;
	...
	TCHAR _leafKeyname[MAX_PATH] = { 0 };
	mbtowc(_leafKeyname, leafKeyName, strlen(leafKeyName));
	res = RegQueryValueEx(hKey, _leafKeyname, 0, NULL, NULL, &size);
	...
	*outptr = ares_malloc(size + 1);
	...
	res = RegQueryValueEx(hKey, _leafKeyname, 0, NULL,
...
static int get_REG_SZ_9X(HKEY hKey, const char *leafKeyName, char **outptr)
{
	...
	*outptr = NULL;
	...
	TCHAR _leafKeyname[MAX_PATH] = { 0 };
	mbtowc(_leafKeyname, leafKeyName, strlen(leafKeyName));
	res = RegQueryValueEx(hKey, _leafKeyname, 0, &dataType, NULL, &size);
	if ((res != ERROR_SUCCESS && res != ERROR_MORE_DATA) || !size)
		return 0;
	...
	*outptr = ares_malloc(size + 1);
	...
	/* Get the value for real */
	res = RegQueryValueEx(hKey, _leafKeyname, 0, &dataType,
		(unsigned char *)*outptr, &size);
	if ((res != ERROR_SUCCESS) || (size == 1))
	{
		ares_free(*outptr);
...
static int get_DNS_Registry_NT(char **outptr)
{
	...
	*/
	...
	res = RegOpenKeyEx(hKey_Tcpip_Parameters, L"Interfaces", 0,
...
static int ip_addr(const char *ipbuf, ssize_t len, struct in_addr *addr)
{
	...
	if (len > 15)
		return -1;
	ULONG s_net_addr;
	ares_inet_pton(AF_INET, ipbuf, &s_net_addr);
	addr->s_addr = s_net_addr;
