...
void CRTSPTransSessionMgrBroadcast::ProcessOperation(CModMessage * pMsg)
{
	INT64 nCallDialogID = -1;
	...
	case  mod_op_t::ot_rtsp::play_broadcast:
	{
		...
		pMsg->SetCallSender(pThreadInfo);
		...
		//	PlayThreadWorker(thisparamInfo);

		//});
		// 启动线程
		///*m_pThread = */AfxBeginThread(pfnTransThread, pthisparamInfo, 0, 0, 0, nullptr);
	}
	break;
	// 其它处理
	case mod_op_t::ot_rtsp::play_ctrl_start_broadcast:
	{
		//pMsg->GetCallDialogID();

		pThreadInfo = static_cast<ThreadInfo_t *>(pMsg->GetCallSender());
		pThreadInfo->oMsgQueue.Push(pMsg);
		break;
	}
...
void CRTSPTransSessionMgrBroadcast::parseBroadcastSdp(CRTSPSession *pSession, CModMessage *pMsg) const
{
...

	{
		...
		else if ("Download" == sdpStack.GetSDPStack().session_name.name)
		{
			...
			auto iter = sdpStack.GetSDPStack().attribute_list.begin();
...
int  CRTSPTransSessionMgrBroadcast::SetupBroadcast(CRTSPSession *pSession, CModMessage *pMsg)
{
	SETUP_INFO tSetupInfo;
	CString strCSeq;
	CString strTrackID;

	parseBroadcastSdp(pSession, pMsg);
...
int CRTSPTransSessionMgrBroadcast::ProcessNetworkEvent(CRTSPSession *pSession, WSAEVENT hNetEvent)
{
...
int CRTSPTransSessionMgrBroadcast::ProcessMsgQueueEvent(CRTSPSession *pSession, ThreadInfo_t* pThreadInfo)
{
	...
	if (pThreadInfo->oMsgQueue.Pop(pMsg, dwWaitTm))
	{
		...
		case  mod_op_t::ot_rtsp::play_ctrl_start_broadcast:
			...
			if (rs_third_200_ret == pSession->eStatus)
			{
				...
				// 开始视频播放的信令交互
				this->SetupBroadcast(pSession, pMsg);
				//	Play(pSession);
...
int CRTSPTransSessionMgrBroadcast::Play(CRTSPSession *pSession)
{
	// 更新状态
